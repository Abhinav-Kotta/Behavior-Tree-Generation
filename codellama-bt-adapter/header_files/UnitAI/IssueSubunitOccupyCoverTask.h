...
//--|
//--| Description:
//--| Defines the FIssueSubunitOccupyCoverTask struct and supporting types.
//--|
//--|====================================================================|--
#pragma once
...
#include "Orders/OccupyCoverOrder.h"
#include "Routes/RoutePoint.h"
#include "UnitAI/WaitForBoundingOverwatchCompletion.h"

// Unreal Engine
#include "CoreMinimal.h"
#include "IssueSubunitOccupyCoverTask.generated.h"

//--------------------------------------------------------------------------------------------------

class URunToCoverOrder;
class UOrder;
class UOrderSubsystem;

//--------------------------------------------------------------------------------------------------

/// @brief Specifies the stages of issuing occupy cover orders to lower echelons.
///
/// @ingroup SimulationBehaviors-Module
UENUM()
enum class EIssueSubunitOccupyCoverOrderStage : uint8
{
...
/// @brief Instance data for @ref FIssueSubunitOccupyCoverTask
///
/// @ingroup SimulationBehaviors-Module
USTRUCT()
struct SIMULATIONBEHAVIORS_API FIssueSubunitOccupyCoverInstanceData
{
    GENERATED_BODY()
    ...
    UPROPERTY()
    EIssueSubunitOccupyCoverOrderStage Stage =
        EIssueSubunitOccupyCoverOrderStage::CREATE_SUBUNIT_ORDER;

    /// @brief The Occupy Cover Order to be issued to subunits
    UPROPERTY()
    TObjectPtr<UOccupyCoverOrder> OccupyCoverOrder;
};

//--------------------------------------------------------------------------------------------------

/// @brief Task for issuing occupy cover orders to SubUnits, Note: This task will only run when
/// executed on a unit with only 2 subunits. The subunits are issued alternating orders which
/// contain a route segment that specifies their next bounding point. This outputs a route to either
/// the right or left units and alternates between the two until bounding overwatch is complete
///
...
struct SIMULATIONBEHAVIORS_API FIssueSubunitOccupyCoverTask : public FMilVerseStateTreeTask
{
    GENERATED_BODY()

    /// @brief Alias for this node's instance data type.
    using FInstanceDataType = FIssueSubunitOccupyCoverInstanceData;
