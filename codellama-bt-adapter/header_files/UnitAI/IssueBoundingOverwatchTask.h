...
//--|
//--| Description:
//--| Defines the FIssueBoundingOverwatchTask struct and supporting types.
//--|
//--|====================================================================|--
#pragma once
...
#include "IssueBoundingOverwatchTask.generated.h"
...
UENUM()
enum class EIssueBoundingOverwatchMovementOrderStage : uint8
{
...
/// @brief Instance data for @ref FIssueBoundingOverwatchTask
///
/// @ingroup SimulationBehaviors-Module
USTRUCT()
struct SIMULATIONBEHAVIORS_API FIssueBoundingOverwatchInstanceData
{
    GENERATED_BODY()

    /// @brief Left Unit's Route.
    UPROPERTY(EditAnywhere, Category = Input)
    TArray<FRoutePoint> LeftRoute;

    /// @brief Right Unit's Route.
    UPROPERTY(EditAnywhere, Category = Input)
    TArray<FRoutePoint> RightRoute;
    ...
    UPROPERTY()
    EIssueBoundingOverwatchMovementOrderStage Stage =
        EIssueBoundingOverwatchMovementOrderStage::CREATE_SUBUNIT_ORDER;

    /// @brief The move tactically order to be issued to subunits
    UPROPERTY()
    TObjectPtr<UMoveTacticallyOrder> MoveTacticallyOrder;
};

//--------------------------------------------------------------------------------------------------

/// @brief Task for issuing movement orders to SubUnits, Note: This task will only run when executed
...
struct SIMULATIONBEHAVIORS_API FIssueBoundingOverwatchTask : public FMilVerseStateTreeTask
{
    GENERATED_BODY()

    /// @brief Alias for this node's instance data type.
    using FInstanceDataType = FIssueBoundingOverwatchInstanceData;
